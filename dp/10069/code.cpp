...
这类字符串问题的状态方程可以从任意一个位置开始往前想（无后效性），比如s1的i和s2的j，如果这两个字母不同，那么d[i][j]=d[i-1][j],即从该字母前去匹配，结果一致；如果字母相同，产生两种情况，1.两个字母i,j已匹配，那么匹配d[i-1][j-1]。2.不用i处字母，匹配s1前i-1个。两种相加即可。
